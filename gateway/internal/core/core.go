...
type Core struct {
	...
	// Boolean indicating whether or not this gateway has been initialised
	Initialised bool
	Ready       chan bool
	...
	// Boolean indicates whether this gateway stores full offer
	StoreFullOffer bool
...
func GetSingleInstance(confs ...*settings.AppSettings) *Core {
	doOnce.Do(func() {
		if len(confs) == 0 {
			logging.Panic("No settings supplied to Gateway start-up")
		}
		if len(confs) != 1 {
			logging.Panic("More than one sets of settings supplied to Gateway start-up")
		}
		instance = &Core{
			Settings:          confs[0],
			Initialised:       false,
