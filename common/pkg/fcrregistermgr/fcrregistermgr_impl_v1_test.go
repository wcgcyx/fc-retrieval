...
func TestUnimplemented(t *testing.T) {
	...
	assert.Empty(t, err)
	assert.Equal(t, uint64(0), maxPage)
	maxPage, err = mgr.GetGWMaxPageByRegion(height, "au")
	assert.Empty(t, err)
	assert.Equal(t, uint64(0), maxPage)
	maxPage, err = mgr.GetPVDMaxPageByRegion(height, "au")
	assert.Empty(t, err)
	assert.Equal(t, uint64(0), maxPage)
