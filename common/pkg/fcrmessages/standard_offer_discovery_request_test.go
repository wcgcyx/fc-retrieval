...
func TestStandardOfferDiscoveryRequest(t *testing.T) {
	...
	msg, err := EncodeStandardOfferDiscoveryRequest(mockNodeID, mockCID, mockNonce, mockMaxOfferRequested, mockAccountAddr, mockVoucher)
	assert.Empty(t, err)
	...
	resNodeID, resCID, resNonce, resMaxOfferRequested, resAcountAddr, resVoucher, err := DecodeStandardOfferDiscoveryRequest(msg)
	assert.Empty(t, err)
